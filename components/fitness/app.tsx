...
export default function FitnessApp({ demo = false }: { demo?: boolean }) {
    ...
    const load = async () => { setError(''); try {
        if (demo) { const data = loadDemo(localStorage); stateRef.current = data; setState(data); return; }
    ...
    const send = async (action: Action) => { if (lock.current || !stateRef.current)
        ...
        let after: AppState;
        if (demo) { after = saveDemo(localStorage, before, action); } else {
        ...
        const result = await r.json() as AppState & {
            error?: string;
        };
        if (!r.ok) {
            if (r.status === 409)
    ...
    useEffect(() => { const mc = (document as any).modelContext; if (!mc?.registerTool)
        return; const lifecycle = new AbortController(); const tool = { name: 'get_training_summary', description: 'Read the current workspace active routine, next day and completed session count. May contain demo data. Read-only.', inputSchema: { type: 'object', properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, untrustedContentHint: true }, execute: async (input: unknown) => { if (!input || typeof input !== 'object' || Object.keys(input).length)
            throw Error('Expected empty object'); const s = stateRef.current, r = s?.routines.find(x => x.status === 'active'); return { content: [{ type: 'text', text: JSON.stringify({ demo, routine: r?.name, nextDay: r?.days[r.nextDay]?.name, completed: s?.sessions.filter(x => x.status === 'complete').length }) }] }; } }; try {
